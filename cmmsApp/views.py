...
def request_demo_view(request):
    ...
    subject = "New Control and Automation Enquiry"
    text_body = (
    ...
    html_body = f"""
        <h2 style="margin:0 0 8px">New Control and Automation Enquiry Request</h2>
...
def gsa(request):    
    return render(request, "gsa.html", {
        "RECAPTCHA_SITE_KEY": settings.RECAPTCHA_SITE_KEY
    })
...
def xgsfd(request):     
    return render(request, "xgsfd.html", {
    "RECAPTCHA_SITE_KEY": settings.RECAPTCHA_SITE_KEY
})
def sheilda(request):     
...
def contact_section(request):
    ...
    if request.method == "POST" and form.is_valid():
        ...
        # Email body
        subject = "New website contact submission for Controls and Automation"
        text_body = "\n".join(
            [
                "New contact submission for Controls and Automation:",
...
def contact_block_submit(request):
   ...
   # <!-- Updated by Diptee on 10-Sep-2026-->
    ...
    # --- Email notification ---
    subject = f"[Control & Automation] Consulting request: {name}"
 
    text_body = "\n".join([
        "A new consulting request was submitted for Control & Automation:",
        f"Name: {name}",
        f"Email: {email}",
